echo "🎮 Ensuring Barotrauma server is running via systemd..."
...
if systemctl is-active --quiet game-startup.service; then
  ...
  echo "🧪 Checking if DedicatedServer is still running before restart:"
  pgrep -a -f DedicatedServer || echo "🔍 No existing DedicatedServer found."
