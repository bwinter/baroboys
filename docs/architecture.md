...
The `cloud_run/vm_control` service is shared infrastructure outside those game workspaces.
It reads a fixed VM-instance mapping and can inspect or start existing VMs; it does not create
or destroy them. Its public Discord interaction endpoint is protected by Discord signature and
guild/channel allowlists. `make bootstrap` establishes and deploys it, while
`make vm-control-deploy` publishes later code changes.
